...
// Local variable.
// We store the token itself and its block depth
//(scope level: 0 is global, 1 is one block in)
typedef struct
{
  Token name;
  int depth;
...
{
  // the function in which this compiler was created
  // that is, the lexically enclosing function
  // of this function
  // The top level function is TYPE_SCRIPT
...
{
  //-2 is to jump the jump offset itself
  // instruction will jump after
  // reading the two offset data bytes
...
{
  // store current as we are leaving it to enter
  // a new function
  ...
  // Compiler claims the first local variable slot
...
static ObjFunction *endCompiler()
{
  ...
  if (!parser.hadError)
  {
    // the top level script is a function
    // with no name (NULL)
...
// starts a new scope on entering a block
...
// walks up the locals array to find the variable
// identifier. If we do not find this, it is a global
static int resolveLocal(Compiler *compiler, Token *name)
{
  // debugLog("Resolving var %1s", name->start);
  for (int i = compiler->localCount - 1; i >= 0; i--)
  {
    ...
    if (identifiersEqual(name, &local->name))
    {
      if (local->depth == -1)
      {
        error("Variable cannot be used in its own initializer.");
      }
      return i;
    }
  }
...
static int resolveUpvalue(Compiler *compiler, Token *name)
{
  ...
  if (upvalue != -1)
  {
    // add the upvalue to this function as well
...
{
  debugLog("Declaring variable in scope %d", current->scopeDepth);
  if (current->scopeDepth == 0)
    return;
  Token *name = &parser.previous;
  // check if the var exists already
...
{
  // do nothing if function is defined globally
  if (current->scopeDepth == 0)
    return;
...
static void function(FunctionType type)
{
  ...
  if (!check(TOKEN_RIGHT_PAREN))
  {
    do
    {
      ...
      {
        errorAtCurrent("Too many parameters (255+)");
      }
...
{
  uint8_t global = parseVariable("Expected function name after 'fun'");
  markInitialized();
...
{
  consume(TOKEN_LEFT_PAREN, "Expected '(' before the if condition.");
  expression();
  consume(TOKEN_RIGHT_PAREN, "Expected ')' after the if condition.");
  // write a placeholder offset for the jump
  int thenJump = emitJump(OP_JUMP_IF_FALSE);
  // then pop the condition expression from stack
  emitByte(OP_POP);
  statement();
  // store jump to skip else branch
  int elseJump = emitJump(OP_JUMP);
  // go back and replace placeholder with actual offset to jump to
  patchJump(thenJump);
  emitByte(OP_POP);
